    ...
    <GlassPanel>
            ...
            {wardrobe.slice(0, 4).map((w) => (
              ...
              >
                ...
                {hoveredId === w.id && canHover() && (
                  <div className="absolute inset-0 bg-foreground/50 flex items-center justify-center p-2">
                    {previewById[w.id]?.status === 'loading' && (
                      <div className="text-background/80 text-xs">Loading preview…</div>
                    )}
                    ...
                    {previewById[w.id]?.status === 'none' && (
                      <div className="text-background/80 text-xs">No try-on yet</div>
                    )}
                    {previewById[w.id]?.status === 'error' && (
                      <div className="text-background/80 text-xs">Preview unavailable</div>
                    )}
                  </div>
                )}
                <div className="absolute inset-0 bg-foreground/60 opacity-0 group-hover:opacity-100 transition-opacity" />
                <div className="absolute bottom-2 left-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <p className="text-background text-xs font-medium truncate mb-2">{w.name}</p>
      ...
      <WardrobeModal open={open} onClose={() => setOpen(false)}>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {wardrobe.map((w) => (
              <div key={w.id} className="group relative rounded-lg overflow-hidden bg-background border border-border hover:border-foreground/50 transition-colors">
                <div className="relative aspect-square overflow-hidden bg-muted">
                  {/* Blurred background */}
                  <img src={w.imageUrl} alt="" className="absolute inset-0 w-full h-full object-cover blur-xl opacity-50" />
                  {/* Main image (fit without crop) */}
                ...
                <div className="p-3">
                  <p className="text-sm font-medium text-foreground mb-2 line-clamp-2">{w.name}</p>
      ...
      <BaseImagePickerModal
        ...
        onSelect={async (base) => {
          ...
          if (!item) return;
          try {
            // Do not change the center image to the base avatar; keep current look until try-on result
